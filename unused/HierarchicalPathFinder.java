...
public class HierarchicalPathFinder<N> implements PathFinder<N> {
	...
	public boolean search (PathFinderRequest<N> request, long timeToRun) {
		if (DEBUG) TheWay.getLogger().debug(TAG, "Enter interruptible HPF; request.status = " + request.status);
		...
		if (request.statusChanged) {
			if (DEBUG) TheWay.getLogger().debug(TAG, "-- statusChanged");
...
//			if (DEBUG) TheWay.getLogger().debug(TAG, "currentLevel = "+levelRequest.currentLevel);

			boolean finished = levelRequestControl.execute(levelRequest);
//			if (DEBUG) TheWay.getLogger().debug(TAG, "finished = "+finished);
//			if (DEBUG) TheWay.getLogger().debug(TAG, "pathFound = "+levelRequest.pathFound);
...
//				levelRequest.pathFound = false;
		...
		if (DEBUG) TheWay.getLogger().debug(TAG, "-- before exit");
	...
	static class LevelPathFinderRequest<N> extends PathFinderRequest<N> {
		...
		public boolean initializeSearch (long timeToRun) {
			...
			do {
				...
				if (DEBUG) TheWay.getLogger().debug(TAG, "LevelPathFinder initializeSearch");
				levelOfNodes = currentLevel;
				currentLevel--;
		...
		public boolean search (PathFinder<N> pathFinder, long timeToRun) {
			if (DEBUG) TheWay.getLogger().debug(TAG, "LevelPathFinder search; status: " + status);
		...
		public boolean finalizeSearch (long timeToRun) {
			...
			if (DEBUG) TheWay.getLogger().debug(TAG, "LevelPathFinder finalizeSearch; status: " + status);
			return true;
		}
